...
close all
if ~exist('t_vec')
    load Single_pop_1_r_1_aligned.mat
end
...
function dydt=ODEfun_2(t,y,d)
y=floor(y);
dydt =4*2*sqrt(pi)*100^(-1)*(100^2-y)*sqrt(1-y/100^2);
end

function dydt=ODE_network(t,y,N,d)
m=reshape(y,N,N);
% m=round(m);
dydt=0.625*reshape((ones(N,N)-m).*(m*diag(ones(N-1,1),-1)+m*diag(ones(N-1,1),1)+...
...
prob=sum(y)/N^2;
% m=round(m);
dydt=4*0.065*reshape((ones(N,N)-m).*prob,N^2,1);
...
m=floor(m);
dydt=4*reshape((ones(N,N)-m).*prob,N^2,1);
...
prob=length(X)/N^2;
% m=round(m);
dydt=4*1*reshape((ones(N,N)-m).*prob,N^2,1);
